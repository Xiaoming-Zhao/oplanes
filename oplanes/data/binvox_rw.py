...
class Voxels(object):
    ...
    def clone(self):
        ...
        scale = self.scale[:]
        return Voxels(data, dims, translate, scale, self.axis_order)
...
def read_header(fp):
    ...
    scale = list(map(float, fp.readline().strip().split(b' ')[1:]))
    line = fp.readline()
...
def write(voxel_model, fp):
    ...
    fp.write(('scale '+' '.join(map(str, voxel_model.scale))+'\n').encode('latin_1'))
    fp.write(b'data\n')
    ...
    for c in voxels_flat:
        if c==state:
            ...
            if ctr==255:
                fp.write(chr(state).encode('latin_1'))
                fp.write(chr(ctr).encode('latin_1'))
                ctr = 0
        else:
